...
public class EmpleadoVista {
    ...
    private void registrar(Scanner scanner) {
        if (servicio.getCantidadEmpleados() >= servicio.getMaximoEmpleados()) {
    ...
    private void mostrarReporte() {
        ...
        Empleado[] lista = servicio.getEmpleados();

        for (var fila = 0; fila < total; fila++) {
            var promedio = servicio.calcularPromedio(fila);
            lista[fila].setPromedioDesempeno(promedio);

            int promedioEntero = (int) promedio;
            System.out.printf("ID: %d | Nombre: %s | Promedio: %d%% ", 
                    lista[fila].getId(), lista[fila].getNombre(), promedioEntero);
